"""
HiddenExtension2 설정값.
V1 대비 변경: 9차원 피처(고도·건물 추가), base r_dim=8, base lam=0.3
"""
import os

LUR_BASE = "/home/data/youngwoong/ST-GNN_Dataset/Data_Preprocessed/Land Use Regression"

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
GRID_DIR = os.path.join(LUR_BASE, "격자 기본")

STGNN_CHECKPOINT = os.path.join(
    BASE_DIR, "../checkpoints/window_12/S3_transport_pm10_pollutants/static/best_model.pt"
)
STGNN_SCENARIO = "S3_transport_pm10_pollutants"
STGNN_WINDOW   = 12

# V1과 동일한 hidden vector / 격자 데이터 공유
HIDDEN_DIR    = os.path.join(BASE_DIR, "../HiddenExtension/data/hidden_vectors")
NDVI_PATH     = os.path.join(GRID_DIR, "ndvi_hourly.npy")
IBI_PATH      = os.path.join(GRID_DIR, "ibi_hourly.npy")
LC_PATH       = os.path.join(GRID_DIR, "landcover_static.npy")
BLDG_PATH     = os.path.join(GRID_DIR, "building_stats_static.npy")  # [G,3] 신규
GRID_CSV_PATH = os.path.join(GRID_DIR, "격자_250m_4326.csv")
TIME_IDX = {
    "train": os.path.join(GRID_DIR, "time_idx_train.npy"),
    "val":   os.path.join(GRID_DIR, "time_idx_val.npy"),
    "test":  os.path.join(GRID_DIR, "time_idx_test.npy"),
}

# 피처 이름 (x_mode='all' 기준 9차원)
FEATURE_NAMES = [
    "NDVI", "IBI",
    "buildings_%", "greenspace_%", "road_struc_%", "river_zone_%",
    "elev_mean", "sum_area", "sum_height",
]

# ── 모델 구조 ────────────────────────────────────────────────────────────────
H_DIM      = 64
X_DIM      = 9   # V2: 6(V1) + 3(고도·건물면적·건물높이)
R_DIM      = 8   # V1 best
ATT_HIDDEN = 32
DROPOUT    = 0.1

# ── Joint training ────────────────────────────────────────────────────────────
LAMBDA = 0.3   # cross 70% 가중 (V1=0.5 보다 cross 강화)

# ── Ablation 설정 ─────────────────────────────────────────────────────────────
# x_mode:
#   'all'         → NDVI+IBI+lc(4)+bldg(3)  9차원
#   'no_building' → NDVI+IBI+lc(4)          6차원 (V1 baseline 비교용)
#   'satellite'   → NDVI+IBI                2차원
#   'static_only' → lc(4)+bldg(3)           7차원
#   'none'        → 0차원
X_MODE    = 'all'
LUR_MODE  = 'linear'
ATTN_MODE = 'full'

# ── 학습 ────────────────────────────────────────────────────────────────────
LR           = 1e-3
WEIGHT_DECAY = 1e-4
EPOCHS       = 200
BATCH_SIZE   = 512
PATIENCE     = 20
