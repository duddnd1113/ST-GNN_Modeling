...
# ──────────────────────────────────────────────────────────
# 2-a. 도로망 기반 후보 필터링 (강/산 제거)
# ──────────────────────────────────────────────────────────

def filter_candidates_to_roads(top_m_idx, df, args):
    """
    OSMnx 도로망 노드를 EPSG:3857(미터 단위)로 투영 후 KD-tree.
    각 후보 셀까지의 실제 미터 거리로 max_road_dist 필터링.
    → 강, 산, 공원 위 격자 자동 제거.

    그리드 셀 간격 참고:
      22500셀 / 서울 bbox ≈ 250-350m 간격
      → max_road_dist 기본값 300m 권장
    """
    if not HAS_OSM:
        logging.warning("osmnx / scipy 미설치 — 도로 필터 건너뜀")
        logging.warning("pip install osmnx scipy")
        return top_m_idx

    if not HAS_MAP:
        logging.warning("pyproj 미설치 — 도로 필터 건너뜀")
        logging.warning("pip install pyproj")
        return top_m_idx

    logging.info("OSMnx 도로망 다운로드 중 (bbox: %.3f~%.3f lon, %.3f~%.3f lat)...",
                 args.bbox_lon_min, args.bbox_lon_max,
                 args.bbox_lat_min, args.bbox_lat_max)

    # osmnx 버전별 API 차이 대응 (1.x / 2.x 모두 호환)
    try:
        # osmnx >= 2.0: keyword args (west, south, east, north)
        G = ox.graph_from_bbox(
            bbox=(args.bbox_lon_min, args.bbox_lat_min,
                  args.bbox_lon_max, args.bbox_lat_max),
            network_type="drive",
        )
    except Exception:
        # osmnx 1.x fallback
        G = ox.graph_from_bbox(
            north=args.bbox_lat_max, south=args.bbox_lat_min,
            east=args.bbox_lon_max,  west=args.bbox_lon_min,
            network_type="drive",
            simplify=True,
        )
    node_gdf = ox.graph_to_gdfs(G, edges=False)
    logging.info("도로 노드 수: %d", len(node_gdf))

    # ── 도로 노드 → EPSG:3857 (미터 단위) ─────────────────
    tr = Transformer.from_crs("EPSG:4326", "EPSG:3857", always_xy=True)
    road_mx, road_my = tr.transform(node_gdf["x"].values, node_gdf["y"].values)
    road_xy_m = np.column_stack([road_mx, road_my])
    road_tree = cKDTree(road_xy_m)

    # ── 후보 셀 → lon/lat → EPSG:3857 ────────────────────
    cand_lons = (args.bbox_lon_min
                 + df.loc[top_m_idx, "x"].values
                 * (args.bbox_lon_max - args.bbox_lon_min))
    cand_lats = (args.bbox_lat_min
                 + df.loc[top_m_idx, "y"].values
                 * (args.bbox_lat_max - args.bbox_lat_min))
    cand_mx, cand_my = tr.transform(cand_lons, cand_lats)
    cand_xy_m = np.column_stack([cand_mx, cand_my])

    # ── 미터 거리로 필터 ──────────────────────────────────
    dists_m, _ = road_tree.query(cand_xy_m)
    keep_mask = dists_m <= args.max_road_dist
    filtered = [idx for idx, keep in zip(top_m_idx, keep_mask) if keep]

    logging.info("도로 필터: %d → %d candidates (max_road_dist=%.0fm)",
                 len(top_m_idx), len(filtered), args.max_road_dist)

    if len(filtered) == 0:
        logging.warning("후보가 0개! max_road_dist(현재 %.0fm)를 늘려보세요.", args.max_road_dist)
...
def main():
    ...
    parser.add_argument("--road-only",      action="store_true",
                        help="도로 근처 후보만 남김 — 강/산 자동 제거 (pip install osmnx scipy 필요)")
    parser.add_argument("--max-road-dist",  type=float, default=300.0,
                        help="도로 노드까지 허용 최대 거리 m (default: 80)")
    ...
    # Step 2-b: 도로망 필터 (강/산 제거)
    if args.road_only:
        top_m_idx = filter_candidates_to_roads(top_m_idx, df, args)
