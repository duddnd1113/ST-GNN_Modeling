...
def evaluate(
    ...
    with torch.no_grad():
        for node_w, edge_w, target, _ in loader:
            ...
            pred, _, __ = model(node_w, edge_index, edge_w)
...
def main(
    ...
    for epoch in epoch_bar:
        ...
        for node_w, edge_w, target, mask in train_loader:
            ...
            optimizer.zero_grad()
            pred, _, __ = model(node_w, edge_index_t, edge_w)
...
def run_all(
    ...
    # 이미 완료된 조합 제외
    pending = [
        (w, s, g) for w, s, g in combos
        if not os.path.exists(os.path.join("checkpoints", f"window_{w}", s, g, "metrics.json"))
    ]
    skipped = total - len(pending)
    print(
        f"총 {total}개 중 {skipped}개 완료(skip), {len(pending)}개 실행 예정 "
        f"({len(windows)}개 윈도우 × {len(scenarios)}개 시나리오 × {len(graph_modes)}개 그래프 모드)\n"
    )

    results = []
    combo_bar = tqdm(pending, desc="실험 전체", unit="exp")
