...
> **Fig D2-4에서 직접 확인됨**: 테스트셋 2,732개 샘플을 풍향별(동/남/서/북)로 분리했을 때,  
> 4방향 모두에서 최상위 attention 엣지 순위가 **완전히 동일**하고 패턴 상관 r > 0.997이었다.  
> wind-aware edge feature가 입력으로 존재함에도 attention이 풍향에 동적으로 반응하지 않는다는  
> 정량적 증거이며, static graph 구조의 근본적 한계를 시각적으로 보여준다.
